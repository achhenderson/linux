			     ...
			     loff_t offset, loff_t len)
{
	...
	if (offset >= 0) {
		 ...
		 * the DLM record is asked about.  A write smaller than a page
		 * marks only its own bytes, so the query has to cover whole
		 * pages or a dirty edge inside a page would not be seen.
		 */
